...
class SecretsManager:
    ...
    def get_secret(self, key: str, default: Optional[str] = None) -> str:
        ...
        if value is None:
            if default is not None:
                ...
                raise ValueError(f"Secret '{self._mask_key(key)}' not found in environment")
        else:
    ...
    def get_jwt_secret(self) -> str:
        ...
        if len(secret) < 32:
            logger.warning(f"JWT secret is too short: {len(secret)} chars (recommended: >= 32)")
        return secret
    ...
    def get_encryption_key(self) -> bytes:
        ...
        if len(key_str) < 32:
            raise ValueError(f"Encryption key too short: {len(key_str)} chars (required: >= 32)")
    ...
    def _validate_secret(self, key: str, value: str):
        ...
        if len(value) < 8 and "key" in key.lower():
            logger.warning(f"Secret '{self._mask_key(key)}' is too short: {len(value)} chars")
